...
- **`_data/strava_stats.yml` is generated** by `scripts/update_strava_stats.py` — don't hand-edit it, the script rewrites the whole file. `_includes/run_stats.html` renders it and no-ops if the file is absent.
...
**Refreshing the Strava totals.** `.github/workflows/strava.yml` runs weekly (and on demand), fetches the totals, and commits `_data/strava_stats.yml`; that push triggers `jekyll.yml` to rebuild. It is deliberately a separate workflow so `jekyll.yml` keeps `contents: read`. Credentials come from the `STRAVA_CLIENT_ID`/`_SECRET`/`_REFRESH_TOKEN` repository secrets. **Strava rotates the refresh token on every refresh and invalidates the old one**, so the workflow writes the new value back with `gh secret set`, which needs a `SECRETS_WRITE_PAT`. Without that, the job succeeds once and fails on its second run.
